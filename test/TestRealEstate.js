...
contract('RealEstate', function(accounts) { 
    ...
    it("가나슈 두번째 계정으로 매물 아이디 0번 매입 후 이벤트 생성 및 매입자 정보와 buyers 배열 테스팅", function() {
        return RealEstate.deployed().then(function(instance) { 
            realEstateInstance = instance;
            return realEstateInstance.buyRealEstate(0, "wkimdev", 23, {from: accounts[1], value: web3.toWei(1.50, "ether")});
            ...
            assert.equal(web3.toAscii(data[1]).replace(/\0/g, ''), "wkimdev", "매입자의 이름이 wkimdev이 아닙니다.");
            assert.equal(data[2], 23, "매입자의 나이가 23살이 아닙니다.");
